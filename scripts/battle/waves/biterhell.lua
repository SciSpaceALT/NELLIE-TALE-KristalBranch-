local biter_3, super = Class(Wave)

function biter_3:onStart()
    ...
    self.timer:every(1/8, function()
		...
		local starspeed = Utils.random(1, 12)
        -- Spawn smallbullet going left with speed 8 (see scripts/battle/bullets/smallbullet.lua)
        --self:spawnBullet("smallbullet", bx, by, math.rad(180), 8)
        --left
        self:spawnBullet("nightmare_dust", leftstarx, leftstary, leftstardir, leftstarspeed)
        --right
        self:spawnBullet("nightmare_dust", rightstarx, rightstary, rightstardir, rightstarspeed)
        --top
        self:spawnBullet("smallstar", topstarx, topstary, topstardir, topstarspeed)  
        --bottom
        self:spawnBullet("nightmare_dust", bottomstarx, bottomstary, bottomstardir, bottomstarspeed)      
        --biter boi
        local by = Utils.random(Game.battle.arena.top, Game.battle.arena.bottom)
