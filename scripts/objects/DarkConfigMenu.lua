...
function DarkConfigMenu:getCurrentItemType()
    if self.item_header_selected == 3 then
        love.graphics.setColor(255,255,255,100)
        love.graphics.print("Days: "..Game:getFlag("Days").."",180,40)
        --love.graphics.print(Game:getFlag("Days"),180,40)
        love.graphics.print(string.format("%02d:%02d", Mod.TimeHours, Mod.TimeMinutesPure),120,80)      
        --love.graphics.print(Mod.TimeHours..":"..Mod.TimeMinutesPure.."",120,80)
        return "POCKETCHATTER"
    end
    if self.item_header_selected == 1 then
        love.graphics.print("The beans will appear here but this is broken rn ",120,40)
        return "POCKETCHATTER"
    end
...
function DarkConfigMenu:getCurrentStorage()
    return Game.inventory:getStorage(self:getCurrentItemType())
end

function DarkConfigMenu:getSelectedItem()
    return Game.inventory:getItem(self:getCurrentItemType(), self.selected_item)
end

function DarkConfigMenu:updateSelectedItem()
    local items = self:getCurrentStorage()
    if #items == 0 then
        self.state = "MENU"
        Game.world.menu:setDescription("Talk to your \nPocket Buddies!", false)
    else
...
function DarkConfigMenu:useItem(item, party)
...
function DarkConfigMenu:update()
    if self.state == "MENU" then
        ...
        if self.item_header_selected == 1 then
            if not self.oneheader then
                self.oneheader = true
                -- create objects
                local dialouge1 = SpeechBubble("CHAOS, CHAOS PUPPET BOY!!!!", 250, 150, none, true)
                local dialouge2 = SpeechBubble("SHUT YOUR [[MouseTrap]]", 150, 150, none, false)
                self:addChild(dialouge1)
                self:addChild(dialouge2)
            end
        else
            self.oneheader = false
        end
    ...
    elseif self.state == "SELECT" then
        ...
        if Input.pressed("confirm") then
            ...
            if self.item_header_selected == 500 then
                self.state = "USE"
...
function DarkConfigMenu:draw()
    if self.item_header_selected == 1 then
        local pocketchair = Assets.getTexture("pocketsprites/chair")
        local pockettable = Assets.getTexture("pocketsprites/table")
        love.graphics.draw(pocketchair, 235, 150)
        love.graphics.draw(pocketchair, 175, 150, 0, -1, 1)
        love.graphics.draw(pockettable, 175, 165)
    else
        --hiya
    end
    love.graphics.setFont(self.font)

    local headers = {"VIEW", "ACTION", "TIME"}
