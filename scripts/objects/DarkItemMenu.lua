...
function DarkItemMenu:getCurrentItemType()
    if self.item_header_selected == 2 then
        return "key_items"
    elseif self.item_header_selected == 4 then
        return "spells"
    else
        return "items"
    end
...
function DarkItemMenu:update()
    if self.state == "MENU" then
        ...
        if self.item_header_selected < 1 then self.item_header_selected = 4 end
        if self.item_header_selected > 4 then self.item_header_selected = 1 end
...
function DarkItemMenu:draw()
    ...
    local headers = {"USE", "KEY", "TBD", "SPELL"}
    ...
    if self.state == "MENU" then
        heart_x = 20 + ((self.item_header_selected - 1) * 120) - 25
        heart_y = 8
    elseif self.state == "SELECT" then
